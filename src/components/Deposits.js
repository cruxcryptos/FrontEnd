	...
	return {
		...
		balance: (
			<Fragment>
				<Box>
					<AmountText
						text={`${formatADXPretty(loyaltyPoolStats.balanceLpADX)} ${"CRUX"} `}
						fontSize={17}
					/>
					{hasExternalStakingTokenTransfers && (
						<span className={classes.info}> *</span>
		...
		allTimeReward: loyaltyPoolStats.totalRewards ? (
			<Box>
				<AmountText
					text={`${formatADXPretty(loyaltyPoolStats.totalRewards)} ${"CRUX"} `}
					fontSize={17}
				/>
				{hasExternalStakingTokenTransfers && (
					<span className={classes.info}> *</span>
		...
		depositsADXTotal: loyaltyPoolStats.totalDeposits ? (
			<Box>
				<AmountText
					text={`${formatADXPretty(loyaltyPoolStats.totalDeposits)} ${"CRUX"} `}
					fontSize={17}
				/>
				{hasExternalStakingTokenTransfers && (
					<span className={classes.info}> *</span>
	...
	return {
		...
		balance: (
			<Fragment>
				<Box>
					<AmountText
						text={`${formatADXPretty(smallPoolStats.balanceLpADX)} ${"CRUX"} `}
						fontSize={17}
					/>
					{hasExternalStakingTokenTransfers && (
						<span className={classes.info}> *</span>
				...
				{
					<Box>{`(${t("deposits.poolShare")}: ${(
						smallPoolStats.userShare * 100
					).toFixed(4)}%)`}</Box>
				}
			</Fragment>
		),
		allTimeReward: smallPoolStats.totalRewards ? (
			<Box>
				<AmountText
					text={`${formatADXPretty(smallPoolStats.totalRewards)} ${"CRUX"} `}
					fontSize={17}
				/>
				{hasExternalStakingTokenTransfers && (
					<span className={classes.info}> *</span>
		...
		depositsADXTotal: smallPoolStats.totalDeposits ? (
			<Box>
				<AmountText
					text={`${formatADXPretty(smallPoolStats.totalDeposits)} ${"CRUX"} `}
					fontSize={17}
				/>
				{hasExternalStakingTokenTransfers && (
					<span className={classes.info}> *</span>
		...
		pendingToUnlockTotalADX: t("common.NA"),
		readyToWithdrawTotalADX: t("common.NA"),
		withdrawsADXTotal: smallPoolStats.totalWithdraws ? (
			<Box>
				<AmountText
					text={`${formatADXPretty(smallPoolStats.totalWithdraws)} ${"ADX"} `}
					fontSize={17}
				/>
				{hasExternalStakingTokenTransfers && (
					<span className={classes.info}> *</span>
...
export default function Deposits() {
	...
	const { loyaltyPoolStats, tomStakingV5PoolStats, smallPoolStats } = stats
